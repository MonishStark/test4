	...
	return (
		<ChartContext.Provider value={{ config }}>
			<div
				...
				{...props}>
				<ChartStyle id={chartId} config={config} /> {/* skipcq: JS-0357 */}
...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
	...
	// Security: Validate and sanitize color values to prevent CSS injection
	...
	// Generate CSS safely with validated inputs
	const cssContent = Object.entries(THEMES)
		.map(([theme, prefix]) => {
			const selectorPrefix = prefix || "";
			const validColorRules = colorConfig
				.map(([key, itemConfig]) => {
					// Sanitize the key name (CSS custom property name)
